...
async function up(phoneNumberSeed: string = '123') {
  const phone = `+1${phoneNumberSeed}4561600`;
  const firstTen = phone.substr(0, 10);
  await make(
    `5-advance-solvency-fail-advance-pass-${phoneNumberSeed}@dave.com`,
    `${firstTen}00`,
    [2, 4],
    5,
  );
  await make(
    `4-advance-solvency-fail-adavnce-fail-${phoneNumberSeed}@dave.com`,
    `${firstTen}01`,
    [2, 3, 4],
    4,
  );
...
async function make(email: string, phoneNumber: string, expenses: number[], numOfAdvances: number) {
  ...
  const advances = [...Array(numOfAdvances).keys()];
  await Bluebird.map(
    advances,
    async (num: number) => {
      await insertFirstAdvance(
        ...
        true,
        75,
        moment().subtract(num + 2, 'months'),
      );
    },
    { concurrency: numOfAdvances + 1 },
  );
}
