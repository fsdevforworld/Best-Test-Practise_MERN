...
async function up(phoneNumberSeed: string = '900') {
  const phone = `+1${phoneNumberSeed}8900000`;
  const firstTen = phone.substr(0, 10);
  await Promise.all([
    make(`express-only0-${phoneNumberSeed}@dave.com`, `${firstTen}00`),
    make(`express-only1-${phoneNumberSeed}@dave.com`, `${firstTen}01`),
    make(`express-only2-${phoneNumberSeed}@dave.com`, `${firstTen}02`),
  ]);
}
...
async function make(email: string, phoneNumber: string) {
  ...
  const { recurringTransactionId } = await insertNormalIncomeTransactions(
    userId,
    bankAccountId,
    { name: 'My Profitable Gambling Habit', amount: 1500, date: payday, period: 'MONTHLY' },
    true,
  );

  await bankAccount.update({ recurringTransactionId });
