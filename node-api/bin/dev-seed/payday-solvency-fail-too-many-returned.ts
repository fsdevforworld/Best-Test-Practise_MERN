...
import { AdminPaycheckOverride, Payment } from '../../src/models';
...
async function up(phoneNumberSeed: string = '123') {
  const phone = `+1${phoneNumberSeed}4566667`;
  await make(
    `solvency-fail-too-many-returned-admin-override-dev-advance5-${phoneNumberSeed}@dave.com`,
    phone,
    [2, 4],
  );
}
