...
describe('subscription collection via synapse (same day ach)', async () => {
  let user: User;
  ...
  it('should collect subscription from bank account', async () => {
    ...
    // fake time to be in the ach window (12AM-9AM PST) for same day
    // hour 10 is in UTC - which is 2AM PST
    const fakeTime = moment().hour(10);
